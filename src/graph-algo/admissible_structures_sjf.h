/*
 * admissible_structures_sjf.h
 *
 *  Created on: January 14, 2013
 *      Author: aousterh
 */

#ifndef ADMISSIBLE_STRUCTURES_SJF_H_
#define ADMISSIBLE_STRUCTURES_SJF_H_
...
struct admission_core_state {
    struct bin *new_request_bins[NUM_BINS]; // pool of backlog bins for incoming requests
    struct bin *outgoing_bins[NUM_BINS];  // pool of bins to go out
    struct bin *temporary_bins[NUM_BINS]; // hold spare allocated bins during run
		...
		struct fp_ring *q_urgent_in, struct fp_ring *q_urgent_out)
{
	...
	for (j = 0; j < NUM_BINS; j++) {
		...
		if (core->new_request_bins[j] == NULL)
			return -1;
                core->outgoing_bins[j] = create_bin(LARGE_BIN_SIZE);
                if (core->outgoing_bins[j] == NULL)
                    return -1;
                core->temporary_bins[j] = NULL;
        }
...
#endif /* ADMISSIBLE_STRUCTURES_SJF_H_ */
