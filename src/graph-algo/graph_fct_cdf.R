...
ggplot(data, aes(fct, group=interaction(algo, as.factor(observed_util)), colour=interaction(algo, as.factor(observed_util)))) + stat_ecdf() +
             labs(x = "Flow Completion Time (timeslots)", y = "Fraction of Samples",
                  title = "CDF of Flow Completion Times") +
             scale_x_log10(breaks= trans_breaks("log10", function(x) 10^x)) + 
             annotation_logticks() + scale_colour_discrete(name="Algorithm/Utilization")