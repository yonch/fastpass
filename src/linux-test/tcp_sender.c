...
void run_tcp_sender_bulk(struct tcp_sender *sender) {
	...
	ret = getsockopt(connection.sock_fd, SOL_SOCKET, SO_ERROR, &result,
			&result_len);
	assert(ret == 0);
	assert(result == 0);
	...
	bcopy((void *) &outgoing, connection.buffer, sizeof(struct packet));
	...
	do {
		...
		connection.return_val = send(connection.sock_fd,
				connection.current_buffer, send_size, 0);
...
void run_tcp_sender_persistent(struct tcp_sender *sender) {
	...
	while ((time_now = current_time_nanoseconds()) < end_time) {
		...
		if (next_send_time > time_now) {
			struct timespec sleeptime = {
				.tv_sec = (next_send_time - time_now) / (1000*1000*1000),
				.tv_nsec = (next_send_time - time_now) % (1000*1000*1000)
			};

			int rc = nanosleep(&sleeptime, NULL);
			assert (rc == 0);
		}
		...
		while (connection.bytes_left > 0) {
			connection.return_val = send(connection.sock_fd, connection.buffer,
						connection.bytes_left, 0);
			if (connection.return_val < 0) {
				printf("sock_fd %d buffer %p bytes_left %lu\n",
						connection.sock_fd, connection.buffer,
						connection.bytes_left);
...
int main(int argc, char **argv) {
	...
	if (argc <= 6 || (type != 0 && type != 1 && type != 2)) {
		printf(
				"usage: %s send_duration mean_t my_id dest_ip size_param type port_num (optional)\n",
				argv[0]);
		return -1;
	}
